...
def fetch_domain(domain):
    ...
    log(f"GET {url} → HTTP {code}", level)

def run_traffic(domains, cycles, delay, threads):
    log(f"Generating traffic: {cycles} cycle(s), {threads} thread(s), {delay}s delay")
    ...
    log("Traffic generation complete", "OK")
...
def main():
    ...
    # ── Traffic generation ───────────────────────────────────────────────
    if args.cycles:
        run_traffic(domains, args.cycles, args.delay, args.threads)
